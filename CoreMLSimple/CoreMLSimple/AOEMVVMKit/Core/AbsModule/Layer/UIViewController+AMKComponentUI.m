...
-(void)amk_viewWillAppear:(BOOL)animated{
    ...
    if (model && [model respondsToSelector:@selector(amk_viewWillAppear:)]) {
...
- (void)amk_viewDidAppear:(BOOL)animated{
    ...
    AMKViewModel * model = (AMKViewModel *)[self valueForKey:@"logicHandler"];
    if (model && [model respondsToSelector:@selector(amk_viewDidAppear:)]) {
...
- (void)amk_viewWillDisappear:(BOOL)animated{
    ...
    AMKViewModel * model = (AMKViewModel *)[self valueForKey:@"logicHandler"];
    if (model && [model respondsToSelector:@selector(amk_viewWillDisappear:)]) {
...
- (void)amk_viewDidDisappear:(BOOL)animated{
    ...
    AMKViewModel * model = (AMKViewModel *)[self valueForKey:@"logicHandler"];
    if (model && [model respondsToSelector:@selector(amk_viewDidDisappear:)]) {
